...
MainInBattleLoop:
  ...
  call EndLowHealthAlarm
	...
	and (1 << NeedsToRecharge ) ; check if the player is using Rage or needs to recharge| (1 << UsingRage)
...
; the player is neither thrashing about nor charging for an attack
	...
	and (1 << StoringEnergy) ; check player is using Bide or using a multi-turn attack like wrap | (1 << UsingTrappingMove)
	jr nz, .selectEnemyMove ; if so, jump
	ld a, [wEnemyBattleStatus1]
	bit UsingTrappingMove, a ; check if enemy is using a multi-turn attack like wrap
	jr .selectPlayerMove ; if not, jump. 
; enemy is using a mult-turn attack like wrap, so player is trapped and cannot execute a move. since commented out usetrapping this should never be called
  ;call BattleRandom
...
.playerDidNotUsePriorityMove
	...
	ld a, [wPlayerSelectedMove]
	cp MIRROR_COAT
	jr nz, .playerDidNotUseMIRROR
	ld a, [wEnemySelectedMove]
	cp MIRROR_COAT
	jr z, .compareSpeed ; if both used mirror
	jr .enemyMovesFirst ; if player used mirror move and enemy didn't
.playerDidNotUseMIRROR
	ld a, [wEnemySelectedMove]
	cp MIRROR_COAT
	jr nz, .playerDidNotUseCounter ; if enemy used mirror and player didn't
	cp COUNTER
...
.handleUnusedBattle
	...
	;ld hl, .RunAwayText ;cutting out some unused stuff to avoid bank too full errors.
	;call PrintText
...
PrintMenuItem:
	...
	ld de, DisabledText
	call PlaceString
	;jr .moveDisabled;pc must be 8 bit? weirdness
.notDisabled
	...
	ld [wcd6d], a
	ld a, [wPlayerSelectedMove]
; print TYPE/<type> and <curPP>/<maxPP>ld a, [wPlayerSelectedMove]
...
.noLinkBattle
	ld a, [wEnemyBattleStatus2]
	and (1 << NeedsToRecharge ) ; need to recharge or using rage | (1 << UsingRage); 
	ret nz
	ld hl, wEnemyBattleStatus1
	ld a, [hl]
	...
	ret nz
	ld a, [wEnemyBattleStatus1]
	and (1 << UsingTrappingMove) | (1 << StoringEnergy) ; using a trapping move like wrap or bide1 
	ret nz
	ld a, [wPlayerBattleStatus1]
	bit UsingTrappingMove, a ; caught in player's trapping move (e.g. wrap)
	jr .canSelectMove ;change jr z to jr for always move? hopefully.
.unableToSelectMove
 ; call BattleRandom
	;cp $c8 ; large chance to escape the wrap temporarily.
	;jp c, .canSelectMove ;hopefully have chance to attack.
	ld a, $ff
	jr .done
.canSelectMove
...
; values for player turn
  ...
  ld a, PSYCHIC
  jr .actualyes
.yes ;should have been named no. oh well.
  ld a, ICE
.actualyes
  ld [wMoveType],a
  jr .contin
...
.checkIfTrapped
	;ld a, [wPlayerBattleStatus1]
	;bit UsingTrappingMove, a ; is the player using a multi-turn attack like wrap
...
.checkIfUsingRage
	ld a, [wEnemyBattleStatus2]
	bit UsingRage, a ; is mon using rage?
	jp .checkEnemyStatusConditionsDone ; if we made it this far, mon can move normally this turn jp z
	ld a, RAGE
	ld [wd11e], a
	;call GetMoveName
	;call CopyStringToCF4B
...
.speedup
  ...
  ;ld [wTemp],a
  call BattleRandom
  cp $e1 ; 1a is 26/256 chance /not/ to have side-effect, is reversed 
  jp c, CantLowerAnymore ;text never displays, but having the jump seems to fix random? or maybe I just had an unlucky string of too many hits...
  ld a, $2
  jr .incrementStatMod
.attackup
  ;ld a,6
  ;ld [wTemp],a ;was going to do something with this, what was it? blerg.
  call BattleRandom
  cp $e1 ; 26/256 chance for side effects
  jp c, CantLowerAnymore
  ld a, $0
  jr .incrementStatMod
.incrementStatMod
	ld c, a
...
.statModifierDownEffect
	...
	jp nz, MoveMissed
	;ld a, [de] ;an attempt at accuracy moves! seems to work OK
