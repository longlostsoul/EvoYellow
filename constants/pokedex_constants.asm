const_value = 1
  ...
  const DEX_PUPITAR
  const DEX_TYRANITAR
  const DEX_BOMBSEEKER;STANTLER
  const DEX_CORSOLA
  const DEX_HOOTHOOT
  const DEX_NOCTOWL
  const DEX_SALANDIT;SUNKERN
  const DEX_SALAZZLE;SUNFLORA
  const DEX_HOPPIP
 const DEX_JUMPLUFF
 const DEX_SKIPLOOM
 const DEX_MAREEP
 ...
 const DEX_REMORAID 
 const DEX_OCTILLERY
 const DEX_RINRIN;SENTRET
 const DEX_BERURUN;FURRET
 const DEX_QWILFISH
 const DEX_TURBAN;DELIBIRD
 const DEX_DUNSPARCE
 const DEX_GIRAFARIG
 const DEX_RIOLU
 const DEX_LUCARIO
